...
def get_ROC_data(x, criterias, num_of_buckets):
    df = pd.DataFrame(x)    
    df["depth"] = criterias["depth"]
    df["read_length"] = criterias["read_length"]
    df["window_size"] = criterias["window_size"]
    df["min_reads"] = criterias["min_reads"]
    df["max_reads"] = criterias["max_reads"]
    df["minimal_score"] = criterias["minimal_score"]
    df["min_HF"] = criterias["min_HF"]
    return(df)
